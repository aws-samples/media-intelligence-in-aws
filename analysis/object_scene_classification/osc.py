...
def lambda_handler(event, context):
    ...
    job_status = start_rekognition_label_job(
        dynamo_record,
        frame_output_path,
    )
...
def start_rekognition_label_job(
    dynamo_record, output_path, start_from="", lambda_arn=""
):
    response = {}
    if S3 is False:
...
def unique_labels_in_image(objects, threshold=80):
    ...
    for label in objects:
        ...
        if object_scene not in unique_objects:
            ...
            unique_objects[object_scene]['avg_confidence'] = get_average(
                unique_objects[object_scene]['avg_confidence'],
                label['Confidence'],
                unique_objects[object_scene]['total_matches']
            )

    return unique_objects


def get_object_scene_labels(
    frame, dynamo_record, batch, min_confidence=70, name_identifier="_frame_"
):
    ...
    try:
        ...
        print(
            "Rekognition label detection exception on file: " + frame + " \n", e
        )
        ...
        if len(job_response['Labels']) > 0:
            individual_results = {
                'S3Key':
                    dynamo_record['S3Key'],
                'AttrType':
                    'ana/osc/' + str(dynamo_record['SampleRate']) +
                    '/{Timestamp}'.format(Timestamp=timestamp),
                'JobId':
                    dynamo_record['JobId'],
                'DetectedLabels':
                    dumps(job_response['Labels']),
                'FrameS3Key':
                    frame
            }
...
def split_objects_scenes(labels):
    ...
    try:
        scene_dictionary_obj = S3.get_object(
            Bucket=environ['DEST_S3_BUCKET'], Key=environ['OSC_DICT']
        )
    except Exception as e:
        print("Failed retreiving dictionary \n", e)
        return False, False, False
    else:
        scene_dictionary = loads(
