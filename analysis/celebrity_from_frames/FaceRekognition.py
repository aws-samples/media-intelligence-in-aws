...
class FaceRekognition:
    ...
    def start_face_search_collection(
        self, collection_id, bucket, video_key, accuracy=80
    ):
        if self.rekognition_client is False:
            print("Failed client, exiting")
            return False
        ...
        try:
            response = self.rekognition_client.start_face_search(
                Video=video, CollectionId=collection_id
            )
        except Exception as e:
            return False
        else:
    ...
    def celeb_names_in_image(self, faces_found, threshold=80, env='dev'):
        ...
        for face_results in faces_found:
            ...
            if face_name not in unique_faces:
                ...
                unique_faces[face_name]['avg_similarity'] = self.get_average(
                    unique_faces[face_name]['avg_similarity'],
                    face_results['Similarity'],
                    unique_faces[face_name]['total_matches']
                )
                unique_faces[face_name]['avg_confidence'] = self.get_average(
                    unique_faces[face_name]['avg_confidence'],
                    face['Confidence'], unique_faces[face_name]['total_matches']
                )
                if env == 'dev':
                    unique_faces[face_name]['face_results'].append(face.copy())
        return unique_faces
    ...
    def detect_faces_in_image(self, bucket, image_key, threshold=0.80):
        ...
        try:
            ...
            print(
                f"Error occurred while detecting faces on image s3://{bucket}/{image_key}"
            )
            return False
        else:
    ...
    def detect_faces_from_collection(
        self,
        collection_id,
        bucket='',
        face_key='',
        ...
        if blob == []:
            image = {'S3Object': {'Bucket': bucket, 'Name': face_key}}
        else:
            image = {'Bytes': blob}
    ...
    def crop_face(self, image, face_start, face_dimensions, defase=40):
        ...
        return new_image[new_face_top[0]:new_face_top[1],
                         new_face_left[0]:new_face_left[1]]
