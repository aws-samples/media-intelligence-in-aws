...
def get_s3_object_list(s3_bucket, path, marker='', s3_objects=[]):
    try:
        ...
        print(
            f'An error occured while listing objects from bucket: {s3_bucket} \n',
            e
        )
        return False
    else:
...
def get_frames_list_s3(s3_bucket, output_path):
    s3_objects = get_s3_object_list(
        s3_bucket, output_path.replace(f's3://{s3_bucket}/', "")
    )
...
def get_brands_from_frame(frame, batch, dynamo_record):
    ...
    frame_timestamp = int(frame.split('.')[-2]
                         ) * 1000 / dynamo_record['SampleRate']
    dynamo_base_name = "ana/bfl/" + str(
...
def detect_brands_from_frames(frames, dynamo_record):
    ...
    with TABLE.batch_writer() as batch:
        with ThreadPoolExecutor(max_workers=5) as pool:
            futures = [
                pool.submit(get_brands_from_frame, frame, batch, dynamo_record)
                for frame in frames
            ]
...
def lambda_handler(event, context):
    ...
    if detect_brands_from_frames(
        get_frames_list_s3(s3_bucket, frame_output_path), dynamo_record
    ):
        ans = LAMBDA.invoke(
    ...
    return SNS_EMAIL_TOPIC.publish(
        Message=" Brand from Logos ready for S3Key: " + message['S3Key'] +
