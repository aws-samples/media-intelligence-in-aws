...
def create_filter(field_type, filter_type):
    if field_type[0] == 'accuracy':
        ...
        return {
            "match": {
                "{}.{}".format(filter_type, field_type[0]): field_type[1]
            }
        }


def search_documents(filters):
    transformed = {"query": {"bool": {"must": []}}}
    ...
    if 'must' in filters.keys():
        transformed['query']['bool']['must'][0]['has_child']['query']['bool'][
            'must'].append(
                {
                    'bool':
                        {
                    ...
                    'bool':
                        {
                            'must_not': query
                            for query in [
                                create_queries(filter_type, 'should')
                                for filter_type in filters['avoid'].items()
                            ]
                        }
                }
            )
    ...
    try:
        print(transformed)
        search = ES_CLIENT.search(index=INDEX_NAME, body=transformed)
        return search
    except Exception as E:
